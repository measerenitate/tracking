...
module('Integration | Component | forms/pairs', function (hooks) {
  ...
  test('it renders', async function (assert) {
    await render(
      hbs`<Forms::Pairs @pairs={{this.pairs}} @onSubmit={{this.dummyAction}} @isSubmitted={{this.isSubmitted}} />`
    );
    ...
    assert
      .dom('[data-test-forms-pairs-entry="breakfast"]')
      .exists({ count: 1 });
  ...
  test('it enables the submit button by default', async function (assert) {
    await render(
      hbs`<Forms::Pairs @pairs={{this.pairs}} @onSubmit={{this.dummyAction}} @isSubmitted={{false}} />`
    );
  ...
  test('it disables the submit button if the @isSubmitted flag is set', async function (assert) {
    await render(
      hbs`<Forms::Pairs @pairs={{this.pairs}} @onSubmit={{this.dummyAction}} @isSubmitted={{true}} />`
    );
  ...
  test('it triggers the onSubmit action if the form is submitted', async function (assert) {
    ...
    await render(
      hbs`<Forms::Pairs @pairs={{this.pairs}} @onSubmit={{this.onSubmit}} @isSubmitted={{this.isSubmitted}} />`
    );
